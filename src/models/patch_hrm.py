"""
patch-hrm: hrm with patchtst-style continuous patch embedding.

inputs are the raw (preprocessed, causally z-scored) series. a non-overlapping
length-`patch_size` linear projection produces `seq_len // patch_size` tokens
that feed the existing working-hrm architecture. no vq-vae, no codebook
collapse, no loss of signed magnitude.
"""
...
class PatchHRM(nn.Module):
    def __init__(self, cfg: PatchHRMConfig):
        super().__init__()
    ...
    def forward(self, raw: torch.Tensor, state: Optional[WorkingHRMState] = None) -> Dict[str, torch.Tensor]:
        # raw: (B, raw_seq_len) continuous
        B, L = raw.shape
        assert L == self.cfg.raw_seq_len, f"expected raw_seq_len={self.cfg.raw_seq_len}, got {L}"
        device = raw.device
