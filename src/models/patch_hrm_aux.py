"""
patch-hrm with a volatility-forecasting auxiliary head.

main head: per-patch UP/DOWN classification (cross-entropy, optionally label-smoothed).
aux head:  per-patch scalar regression of forward realized log-volatility.
the shared hrm trunk is regularized by having to also explain the magnitude
structure, which is typically much more predictable than direction.
"""
...
class PatchHRMAux(nn.Module):
    def __init__(self, cfg: PatchHRMAuxConfig):
        super().__init__()
        ...
        self.vol_head = nn.Linear(cfg.hidden_size, 1, bias=True)
