"""
train patch-hrm: hrm with continuous patch embedding (no vq-vae).

uses the same temporal split + embargo as train_and_compare_all.py so results
are directly comparable. windows are non-overlapping length-64 segments of the
preprocessed series; each contributes 16 patch-level UP/DOWN labels.
"""
...
from src.models.patch_hrm import PatchHRM, PatchHRMConfig
...
def build_windows(features: np.ndarray, labels: np.ndarray, start: int, stop: int):
    """non-overlapping length-64 windows fully inside [start, stop). per-window label is
    one value per patch (the last raw label inside each patch)."""
...
def iter_batches(x: torch.Tensor, y: torch.Tensor, batch_size: int, shuffle: bool, seed: int):
    n = x.shape[0]
    idx = np.arange(n)
    if shuffle:
...
def main():
    ...
    p.add_argument("--out_csv", type=str, default="results/patch_hrm.csv")
    args = p.parse_args()
    ...
    print(f"train label balance (non-ignore): "
          f"{(y_tr[y_tr != IGNORE_LABEL] == 1).float().mean():.3f}")
    print(f"test  label balance (non-ignore): "
          f"{(y_te[y_te != IGNORE_LABEL] == 1).float().mean():.3f}")
    ...
    model = PatchHRM(cfg).to(device)
    n_params = sum(p.numel() for p in model.parameters())
    print(f"patch-hrm params: {n_params:,}")

    opt = torch.optim.AdamW(model.parameters(), lr=args.lr, weight_decay=0.01)
    sched = torch.optim.lr_scheduler.CosineAnnealingLR(opt, T_max=args.epochs)

    best_loss = float('inf')
    ckpt_path = "checkpoints_patch/best_patch_hrm.pt"
    ...
    metrics = evaluate(model, x_te, y_te, device, args.batch_size)
    metrics['params'] = n_params
    metrics['model'] = 'Patch-HRM'
    print("\nheld-out test:", metrics)

    os.makedirs(os.path.dirname(args.out_csv), exist_ok=True)
    pd.DataFrame([{
        'Model': 'Patch-HRM', 'Params': f"{n_params:,}", 'TestN': metrics['n'],
        'Loss': f"{metrics['loss']:.4f}", 'Accuracy': f"{metrics['accuracy']:.4f}",
        'Precision': f"{metrics['precision']:.4f}", 'Recall': f"{metrics['recall']:.4f}",
        'F1': f"{metrics['f1']:.4f}", 'MCC': f"{metrics['mcc']:.4f}",
